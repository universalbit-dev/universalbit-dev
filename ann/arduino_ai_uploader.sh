#!/usr/bin/env bash
# ==============================================================================
# arduino_ai_uploader.sh
# TinyML / Light AI Compiler & Flasher Tool (Dynamic Sandbox Edition)
# Supports Arduino Nano/Uno, ESP8266 (D1 Mini), and ESP32
# ==============================================================================
...
echo -e "${GREEN}========================================================${NC}"
echo -e "${GREEN}   [Light AI / TinyML] Arduino AI Uploader Tool         ${NC}"
echo -e "${GREEN}========================================================${NC}"
...
if [ ${#sketches[@]} -eq 0 ]; then
    ...
    echo -e "Please ensure ArduinoANN.ino or ArduinoRL.ino is in: $(pwd)${NC}"
    exit 1
fi

echo -e "${YELLOW}Select the AI model sketch to compile and deploy:${NC}"
...
if [ "$DETECTED_CHIP" = "esp8266" ]; then
    echo -e "${GREEN}✔ Auto-detected Hardware: ESP8266 board (using D1 Mini profile)${NC}"
    ...
    echo "1) Arduino Nano"
    echo "2) Arduino Nano (Old Bootloader / Common for clones)"
    echo "3) Arduino Uno R3 / Classic"
    echo "4) ESP8266 (e.g. Wemos D1 Mini)"
    echo "5) ESP32 Dev Module (Stable 2.x Core)"
    read -rp "Selection [1-5]: " BOARD_CHOICE

    case "$BOARD_CHOICE" in
        1) FQBN="arduino:avr:nano"; CORE="arduino:avr" ;;
        2) FQBN="arduino:avr:nano:cpu=atmega328old"; CORE="arduino:avr" ;;
        3) FQBN="arduino:avr:uno"; CORE="arduino:avr" ;;
        4) FQBN="esp8266:esp8266:d1_mini"; CORE="esp8266:esp8266" ;;
        5) FQBN="esp32:esp32:esp32"; CORE="esp32:esp32@2.0.17" ;;
