#!/usr/bin/env bash
# ==============================================================================
# esp8266_ntp.sh - Automated Build, Flash Deployment & Serial Diagnostics
# Platform Target: ESP8266 (Wemos D1 Mini / NodeMCU / D1 R3)
# ==============================================================================
set -Eeuo pipefail
...
# Detect running context to handle configuration paths correctly
...
install_esptool_dependency() {
  if require_cmd esptool; then return 0; fi
  warn "esptool is missing."
  read -rp "Install esptool now via apt/pip? [y/N]: " ans
  case "${ans:-N}" in
    y|Y|yes|YES)
      step "Attempting to install esptool..."
      if require_cmd apt-get; then
        sudo apt-get update && sudo apt-get install -y esptool
      elif require_cmd pip3; then
        pip3 install esptool
      elif require_cmd pip; then
        pip install esptool
      else
        err "Could not find apt or pip. Please install esptool manually."
        exit 1
      fi
      ;;
    *) err "esptool is required for chip operations. Exiting."; exit 1 ;;
  esac
}
...
install_arduino_cli_linux() {
  ...
  case "${ans:-N}" in
    y|Y|yes|YES)
      ...
      # Use sudo context translation so configurations don't default to /root
      sudo -u "$REAL_USER" -H "$LOCAL_BIN/arduino-cli" config init --overwrite
      sudo -u "$REAL_USER" -H "$LOCAL_BIN/arduino-cli" config set board_manager.additional_urls https://github.com/esp8266/Arduino/releases/download/3.1.2/package_esp8266com_index.json
      ;;
...
ensure_esp8266_core() {
  ...
  # Handle updating both root index and local home context configurations
  arduino-cli config set board_manager.additional_urls https://github.com/esp8266/Arduino/releases/download/3.1.2/package_esp8266com_index.json
...
run_esptool_flash_id() {
  ...
  warn "flash-id default reset failed, retrying with no-reset control lines..."
  if esptool --port "$port" --before no-reset --after no-reset flash-id >/dev/null 2>&1; then
    return 0
  fi
  return 1
}
...
flash_bin() {
  local port="$1"
  local fw="$2"
  # Updated choices to avoid esptool v5.x deprecation warnings
  local before_mode="${ESPTOOL_BEFORE:-default-reset}"
  local after_mode="${ESPTOOL_AFTER:-hard-reset}"
  ...
  warn "Flash failed with default-reset strategy; retrying no-reset + lower baud..."
  if esptool --chip esp8266 --port "$port" \
  ...
  err "Flash failed in both normal and fallback modes."
  return 1
}
...
open_monitor() {
  ...
  if require_cmd picocom; then
    ...
    warn "Standard terminal monitors (picocom, minicom, screen) not found."
    log "Falling back to built-in arduino-cli monitor..."
    arduino-cli monitor -p "$port" --config "baudrate=$BAUD_MONITOR"
  fi
}
